...
const FilterPanel = ({ filters, onChange }) => {
  const handleSourceChange = (sourceVal) => {
    onChange({ ...filters, formSource: filters.formSource === sourceVal ? '' : sourceVal });
  };
  ...
  return (
    <div className="bg-[#141517] border border-border-card rounded-2xl p-4 shadow-lg flex flex-col justify-between select-none">
      <div className="flex items-center justify-between border-b border-[#222326] pb-1.5 mb-3">
        <h3 className="text-xs font-bold tracking-widest text-text-secondary uppercase text-left">Filtrar por Origem</h3>
        {filters.formSource && (
          <button onClick={() => onChange({ ...filters, formSource: '' })}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {['Formulário', 'MQL', 'Direct', 'Story Dr'].map(source => (
          <button key={source} onClick={() => handleSourceChange(source)}
