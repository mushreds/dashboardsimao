    ...
    <div className="flex bg-[#141517] border border-border-card rounded-2xl p-4 md:p-5 shadow-lg relative h-full flex-col md:flex-row gap-4">
      <div className="flex md:hidden items-center justify-between border-b border-[#222326] pb-2.5 mb-1 select-none">
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-[9px] font-extrabold text-gold-primary leading-none mono-numbers">{totalLeads || 0}</span>
          <span className="text-[8px] text-text-muted font-bold tracking-widest uppercase leading-none">Leads</span>
        </div>
      </div>

      <div className="hidden md:flex flex-col items-center justify-between border-r border-[#222326] pr-4 w-12 select-none">
        <span className="text-xs font-black tracking-[0.3em] text-white uppercase transform -rotate-90 origin-center my-auto whitespace-nowrap">{title}</span>
        <div className="flex flex-col items-center gap-1 mt-2">
          <span className="text-[9px] font-bold text-gold-primary tracking-wider leading-none mono-numbers">{totalLeads || 0}</span>
          <span className="text-[7px] text-text-muted font-bold tracking-widest uppercase leading-none">Leads</span>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-between gap-3">
        ...
        <div className="flex flex-col justify-between gap-2.5 flex-1">
          {stages && stages.map((stage, idx) => {
            ...
            if (idx > 0 && stages[idx - 1].count > 0) relativeConvRate = (stage.count / stages[idx - 1].count) * 100;
            else if (idx > 0) relativeConvRate = 0;
              ...
              <div key={stage.name} className="flex justify-between items-center text-xs h-8 group gap-1">
                ...
                <div className="flex-1 flex justify-start items-center px-1 sm:px-4 relative">
                  <div style={{ width: `${widthPercent}%` }}
                    className="h-6 sm:h-7 bg-[#2A2115]/20 border border-[#2A2115] hover:border-gold-primary transition-all duration-500 ease-out rounded-lg flex items-center justify-end px-2 sm:px-3 shadow-inner relative">
                    <div className="absolute inset-y-0 left-0 bg-gradient-to-r from-[#2A2115] to-[#817566] rounded-l-lg opacity-25" style={{ width: '30%' }} />
                    <span className="text-[9px] sm:text-[10px] font-extrabold text-white z-10 mono-numbers">{stage.count}</span>
                  </div>
                </div>
                ...
                <span className="w-16 sm:w-20 text-right text-[8px] sm:text-[10px] font-bold text-gold-primary select-none mono-numbers">
                  {`${relativeConvRate.toFixed(1).replace('.', ',')}%`}
                </span>
              </div>
            );
          })}
